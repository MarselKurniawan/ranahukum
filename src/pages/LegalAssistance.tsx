...
export default function LegalAssistance() {
  ...
  const filteredLawyers = mockLawyers.filter((lawyer) => {
    const matchesSearch = lawyer.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      lawyer.location.city.toLowerCase().includes(searchQuery.toLowerCase());
    
    const matchesProvince = selectedProvince === "Semua Provinsi" || 
      lawyer.location.province === selectedProvince;

    const matchesSpec = selectedSpec === "Semua" ||
      lawyer.specializations.includes(selectedSpec);

    return matchesSearch && matchesProvince && matchesSpec && lawyer.pendampinganPrice;
  });
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-muted-foreground">
                          • {lawyer.experience} tahun pengalaman
                        </span>
                      </div>
